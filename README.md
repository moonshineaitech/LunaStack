<p align="center">
  ...
  <em>933 skills · 251 core protocols + a 682-skill specialist wiki · 29 domains · 55 roles</em>
</p>
...
<p align="center">
  ...
  251 core protocols <em>plus</em> a 682-skill verified specialist wiki spanning<br>
  29 domains, from frameworks and databases to personal health and agent engineering.<br>
...
./setup.sh --pack security     # +19 skills: AI red-team, audits, supply chain, skill safety
...
./setup.sh --global            # everything (all 251)
```
...
library**: [`wiki/skills/`](wiki/skills/README.md) — **682 behavior-grade
specialist skills across 29 domains**, from `/postgres-production-tuning` to
`/context-engineering` to `/multiplayer-netcode` to `/mental-health-support`.
...
- **Live**: second-brain (PKM & life ops) · finance (education-only) · [health](wiki/skills/health/README.md) — 133 personal-health skills
...
### 🛡️ Security Skills (10 protocols)
Hardened development from Trail of Bits + CVE lessons, plus authorized AI/LLM security — `/red-team` (OWASP LLM Top 10, authorization-gated), `/prompt-injection-defense` (trust-boundary hardening), `/cve-scan`, `/supply-chain-audit`, `/codeql-semgrep`, `/threat-db`, `/malicious-skill-detection`, `/sbom`, `/dependency-typosquat`, `/secret-rotation-plan`.

---

## What's Inside

**The core methodology — 27 disciplines · 251 protocols · 55 specialist roles.**
_(Plus the 682-skill specialist [wiki](#the-skill-wiki) — that's the "what a specialist knows" layer.)_
...
| 🛡️ **Security Skills** | 10 | `/cve-scan` `/supply-chain-audit` `/threat-db` |
...
| File | Purpose | Size |
|:---|:---|:---|
| `LunaStack.md` | **The single file.** Upload to Claude Project. Everything works. Generated by `build.sh` — never hand-edited. | 344KB |
| `*/SKILL.md` | 251 core protocol files for Claude Code CLI — the source of truth for the single-file build | — |
| `wiki/skills/` | 682-skill verified specialist library across 29 domains — browse [`INDEX.md`](wiki/skills/INDEX.md) | — |
| `build.sh` | Regenerates `LunaStack.md` from the skill files | 3KB |
| `AGENTS.md` | Complete skill catalog — also drives the build's discipline grouping | — |
| `setup.sh` | Symlinks all skills into `~/.claude/skills/` | 1KB |
| `uninstall.sh` | Removes symlinks | 0.7KB |
| `tests/` | Structural validation, integrity checks, quality scoring | — |

---

## Start Here

Don't try all 251 protocols at once. Start with these 7:
...
<p align="center">
  ...
  <sub>933 skills · 251 core protocols + 682-skill wiki · 29 domains · 55 roles · MIT</sub>
</p>
