...
while IFS= read -r f; do
  ...
  grep -qiE 'licensed|clinician|doctor|physician|healthcare (professional|provider)|pharmacist|professional|surgeon|surgical team|care team|prescriber|specialist|allergist|dietitian|nurse|counselor|therapist|attorney|advocate|dentist|dermatologist|optometrist|ophthalmologist|podiatrist|eye doctor|eye-care|travel clinic|pediatrician|obstetrician|midwife|audiologist|oncolog|nephrolog|neurolog|cardiolog|urolog|gastroenterolog|psychiatr' "$f" \
